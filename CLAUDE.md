...
**Staged answer reveal.** Tapping an option colours *that option only*, plays the matching cue from [sounds.js](src/data/sounds.js), and reveals the correct answer after `REVEAL_DELAY` (450ms correct / 900ms wrong, in [Quiz.jsx](src/screens/Quiz.jsx)). Revealing both at once reads as one confusing blink — the visitor needs a beat to register their own pick first. Next is disabled during that window so a fast tap cannot skip the reveal, and the un-picked options stay neutral rather than dimming early, which would give the answer away. `optionState` takes `showPick` and `showAnswer` separately for this reason; collapsing them back into one flag reintroduces the simultaneous flash. The reveal flag stores the question id, not a boolean, so changing question invalidates it without an effect resetting state.
